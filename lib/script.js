'use strict';
  ...
  /**
 * Script for selects-related Input Plugin
 *
 ...
 * @param {Object} input.parent - parent of input and it's attributes
 ...
 *
 * @module selectsRelatedScripts
 */

module.exports = function selectsRelatedScript(input) {
  input.target.addEventListener('change', function (e) {
    e.target.parentNode.querySelectorAll('select').forEach(function (select) {
      // browser based script requires var
      var key = select; // eslint-disable-line no-var
      key.selectedIndex = e.target.selectedIndex;
    });
  });
